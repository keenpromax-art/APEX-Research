...
async function main() {
  ...
  console.log(`Document has ${children.length} pages declared. Checking each individually...`);

  for (let i = 0; i < children.length; i++) {
    const singleDoc = React.createElement(Document, null, children[i]);
    ...
    if (count > 1) {
      console.log(`❌ PAGE ${i + 1} OVERFLOWED! Produced ${count} pages! Component: ${children[i].type.name || 'Anonymous'}`);
    } else {
      console.log(`✓ Page ${i + 1}: ${count} page`);
    }
  }
}
